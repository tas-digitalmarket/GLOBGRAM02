...
class RoomRemoteDataSourceLocal {
  ...
  static final Map<String, StreamController<RTCIceCandidate>> _iceCandidateControllers = {};
  ...
  Future<Map<String, dynamic>?> getRoomData(String roomId) async {
    try {
      ...
      _logger.d('Room data retrieved for $roomId: ${roomData != null ? 'found' : 'not found'}');
      return roomData;
    } catch (e) {
  ...
  /// Add ICE candidate
  Future<void> addIceCandidate(String roomId, String type, RTCIceCandidate candidate) async {
    try {
      ...
      _rooms[roomId]!['iceCandidates'][type] = _rooms[roomId]!['iceCandidates'][type] ?? [];
  ...
  Stream<RTCIceCandidate> listenIceCandidates(String roomId, String type) {
    ...
    if (!_iceCandidateControllers.containsKey(key)) {
      _iceCandidateControllers[key] = StreamController<RTCIceCandidate>.broadcast();
    }
